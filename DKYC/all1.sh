...
awk -F'[|,;\t]' '{print $'$a',$'$b',$'$c',$'$d'}' OFS='|' samples_pre_dkyc.txt >reduced_pre_dkyc.txt
mv reduced_pre_dkyc.txt columns_reduced
awk -F'[|,;\t]' '{print $'$a',$'$b',$'$c',$'$d'}' OFS='|' samples_post_dkyc.txt >reduced_post_dkyc.txt
mv reduced_post_dkyc.txt columns_reduced

awk -F'[|,;\t]' '{print $'$a',$'$b',$'$c',$'$d'}' OFS='|' samples_pre_reverified.txt >reduced_pre_reverified.txt
mv reduced_pre_reverified.txt columns_reduced
awk -F'[|,;\t]' '{print $'$a',$'$b',$'$c',$'$d'}' OFS='|' samples_post_reverified.txt >reduced_post_reverified.txt
mv reduced_post_reverified.txt columns_reduced
...
cat *.txt >dbmerged.txt
cd ..
echo pre_ekyc pre_paper pre_dkyc pre_reverified post_ekyc post_paper post_dkyc post_reverified bulk_ekyc bulk_paper  | xargs -n 1 cp mdn.txt # copying history file (mdn.txt) to all 6 folders
...
mv samples_pre_ekyc.txt ../results # move sampled file to results folder
...
cd pre_dkyc
point_one
echo -e "\n\n$(tput setaf 4) $(tput setb 7)total number of prepaid dkyc subscribers = $q. 0.1% sample count should be  $t$(tput sgr0)\n"
sampler
mv p2.txt p2_pre_dkyc.txt
mv p2_pre_dkyc.txt ../numbers
mv samples.txt samples_pre_dkyc.txt
mv samples_pre_dkyc.txt ../results
find . ! -name 'dbmerged.txt' -type f -exec rm -f {} +
mv dbmerged.txt database.txt
cd ..
...
cd pre_reverified
point_one
echo -e "\n\n$(tput setaf 4) $(tput setb 7)total number of prepaid reverified subscribers = $q. 0.1% sample count should be  $t$(tput sgr0)\n"
sampler
mv p2.txt p2_pre_reverified.txt
mv p2_pre_reverified.txt ../numbers
mv samples.txt samples_pre_reverified.txt
mv samples_pre_reverified.txt ../results
find . ! -name 'dbmerged.txt' -type f -exec rm -f {} +
mv dbmerged.txt database.txt
cd ..
...
echo -e "\n\n$(tput setaf 4) $(tput setb 7)total number of prepaid dkyc subscribers = $q. 0.1% sample count should be  $t$(tput sgr0)\n"
sampler
mv p2.txt p2_post_dkyc.txt
mv p2_post_dkyc.txt ../numbers
mv samples.txt samples_post_dkyc.txt
mv samples_post_dkyc.txt ../results
find . ! -name 'dbmerged.txt' -type f -exec rm -f {} +
mv dbmerged.txt database.txt
cd ..
...
echo -e "\n\n$(tput setaf 4) $(tput setb 7)total number of prepaid reverified subscribers = $q. 0.1% sample count should be  $t$(tput sgr0)\n"
sampler
mv p2.txt p2_post_reverified.txt
mv p2_post_reverified.txt ../numbers
mv samples.txt samples_post_reverified.txt
mv samples_post_reverified.txt ../results
find . ! -name 'dbmerged.txt' -type f -exec rm -f {} +
mv dbmerged.txt database.txt
cd ..
